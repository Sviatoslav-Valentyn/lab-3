...
public class Service {
    ...
    public void sortGoodsByAlphabet() {
        Map<Good, Integer> sortedGoods = save.getWarehouse().getGoods().entrySet().stream().sorted((e1, e2) -> {
        ...
        save.getWarehouse().setGoods(sortedGoods);
    }
    ...
    public double getAveragePrice() {
        return save.getWarehouse().getGoods().entrySet().
                stream().mapToDouble(p -> p.getKey().getSellingPrice()).average().orElse(Double.NaN);
    }

    public double getTotalSpendByClient(final Client client, final LocalDate from, final LocalDate to) {
        AtomicReference<Double> result = new AtomicReference<>((double) 0);
        save.getHistory().getReceipts().stream().forEach((receipt) -> {
            if (receipt.getClient().equals(client) && (receipt.getDate().
    ...
    public void filterGoodsByMinAndMax(final double min, final double max) {
        Map<Good, Integer> sortedGoods = save.getWarehouse().getGoods().entrySet().
                stream().filter(e -> e.getKey().getSellingPrice() >= min && e.getKey().
                        getSellingPrice() <= max).collect(Collectors.toMap(p -> p.getKey(), p ->
                        p.getValue(), (v1, v2) -> v1, LinkedHashMap::new));

        save.getWarehouse().setGoods(sortedGoods);
    }

    public void getAllGoodsByClient(final Client client) {
        LinkedList<Receipt> receipts = save.getHistory().getReceipts();
        LinkedList<Receipt> filteredReceipts = receipts.stream().filter(e -> e.getClient().
                equals(client)).collect(Collectors.toCollection(LinkedList::new));
        ...
        Map<String, Long> resultCount = allGoods.
                stream().collect(Collectors.groupingBy(Good -> "Name: " +
                        Good.getName() + "; Manufacturer: " +
                        Good.getManufacturer(), Collectors.counting()));
    ...
    public void getBiggestIncome() {
        ...
        Map<LocalDate, Double> entryCalculateReceipt = receipts.
                stream().collect(Collectors.toMap(p -> p.getDate(), p -> {
            AtomicReference<Double> res = new AtomicReference<>((double) 0);
            p.getGoods().entrySet().stream().forEach(g -> res.set((g.getKey().
                    getSellingPrice() - g.getKey().getBuyingPrice()) * g.getValue()));
            return res.get();
        ...
        Map<LocalDate, Double> sortedDates = entryCalculatedByDate.entrySet().
                stream().sorted(Map.Entry.comparingByValue()).collect(Collectors.
                        toMap(p -> p.getKey(), p -> p.getValue(), (v1, v2) -> v1, LinkedHashMap::new));
    ...
    public void getMostPopularGood() {
        ...
        Map<String, Long> resultCount = allGoods.stream().collect(groupingBy(Good ->
                "Name: " + Good.getName() + "; Manufacturer: " + Good.getManufacturer(), Collectors.counting()));

        Map<String, Long> sortedGoods = resultCount.entrySet().stream().
                sorted(Map.Entry.comparingByValue()).collect(Collectors.toMap(p ->
                        p.getKey(), p -> p.getValue(), (v1, v2) -> v1, LinkedHashMap::new));
